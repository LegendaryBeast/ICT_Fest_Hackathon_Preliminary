...
This report documents the 43 unique, verified bugs identified and resolved in the CoWork API codebase. Each entry details the file location, the nature of the issue, why it caused incorrect behavior, and how it was fixed to satisfy the API contract.
...
### Bug 13 — Refund 48-Hour Boundary Uses Strict `>` Instead of `>=`
...
### Bug 14 — Refund `< 24h` Returns 50% Instead of 0%
...
### Bug 15 — Refund Rounding Uses Truncation Instead of Half-Up
...
### Bug 16 — Refund Amount Inconsistency Between Response and Log
...
### Bug 17 — List Bookings Sorted Descending Instead of Ascending
...
### Bug 18 — Pagination Offset Wrong (Off by One)
...
### Bug 19 — Pagination Limit Hardcoded to 10
...
## 6. Multi-Tenancy & Visibility (3 bugs)

### Bug 20 — Member Can Read Another Member's Booking
*   **File Location:** `app/routers/bookings.py`
...
### Bug 21 — Admin List Bookings Missing Org Scope
*   **File Location:** `app/routers/bookings.py`
...
### Bug 22 — Export Cross-Organization Data Leak
...
## 7. Database & Data Integrity (2 bugs)

### Bug 23 — Missing Unique Constraint on Reference Code
...
### Bug 24 — Database Session Missing Rollback on Exception
...
### Bug 25 — Reference Code Counter Not Atomic
...
### Bug 26 — Rate Limiter Not Concurrency-Safe
...
### Bug 27 — Room Stats Use Inconsistent In-Memory Cache
...
### Bug 28 — Double-Booking Race Condition
*   **File Location:** `app/routers/bookings.py`
...
### Bug 30 — Double Cancel Race Condition (Multiple Refunds)
*   **File Location:** `app/routers/bookings.py`, `app/services/refunds.py`
...
### Bug 31 — Notification Lock-Order Deadlock
...
### Bug 32 — Artificial Sleeps Serializing All Request Paths
*   **File Locations:**
...
## 10. Cache & Reporting (4 bugs)

### Bug 33 — Cache Module No Thread Safety
...
### Bug 34 — Usage Report Stale After Booking Creation
*   **File Location:** `app/routers/bookings.py`
...
### Bug 35 — Availability Cache Stale After Cancellation
*   **File Location:** `app/routers/bookings.py`
...
### Bug 36 — Usage Report Accepts Inverted Date Range Silently
...
## 11. Robustness & Memory Safety (1 bug)

### Bug 37 — Revoked Token Set Grows Without Bound
...
## 12. Unhandled Errors & Formats (2 bugs)

### Bug 38 — Malformed Datetime Strings Cause Unhandled 500
*   **File Location:** `app/routers/bookings.py`
...
### Bug 40 — Stale-Cache TOCTOU Race Condition in Usage-Report and Availability Caching
...
### Bug 41 — Broken Notifications Import in Bookings Router
*   **File Location:** `app/routers/bookings.py`
...
### Bug 42 — Admin `list_bookings` Scope Conflict with Rule 11
*   **File Location:** `app/routers/bookings.py`
...
| 12 | Booking | `app/routers/bookings.py` | start_time overwritten with created_at |
| 13 | Refund | `app/routers/bookings.py` | 48h boundary uses > instead of >= |
| 14 | Refund | `app/routers/bookings.py` | <24h notice gets 50% instead of 0% |
| 15 | Refund | `app/services/refunds.py` | Float truncation instead of half-up |
| 16 | Refund | `app/routers/bookings.py` | Response and log amounts diverge |
| 17 | Pagination | `app/routers/bookings.py` | Sorted descending instead of ascending |
| 18 | Pagination | `app/routers/bookings.py` | Offset skips first page |
| 19 | Pagination | `app/routers/bookings.py` | Limit hardcoded to 10 |
| 20 | Tenancy | `app/routers/bookings.py` | Members read other members' bookings |
| 21 | Tenancy | `app/routers/bookings.py` | Admin can't see org-wide bookings |
| 22 | Tenancy | `app/services/export.py` | Cross-org export data leak |
| 23 | Data | `app/models.py` | reference_code missing unique=True |
| 24 | Data | `app/database.py` | Session not rolled back on exception |
| 25 | Concurrency | `app/services/reference.py` | Counter race → duplicate codes |
| 26 | Concurrency | `app/services/ratelimit.py` | Rate limiter race → bypassed |
| 27 | Concurrency | `app/services/stats.py` | In-memory stats lost on restart/race |
| 28 | Concurrency | `app/routers/bookings.py` | Check-then-insert → double booking |
| 29 | Concurrency | `app/routers/bookings.py` | Quota bypass under concurrency |
| 30 | Concurrency | `app/routers/bookings.py` | Double cancel → multiple refunds |
| 31 | Concurrency | `app/services/notifications.py` | AB-BA deadlock on create vs cancel |
| 32 | Performance | Multiple files | Artificial sleeps inside critical locks |
| 33 | Cache | `app/cache.py` | Dict mutations without lock |
| 34 | Cache | `app/routers/bookings.py` | Create doesn't invalidate report cache |
| 35 | Cache | `app/routers/bookings.py` | Cancel doesn't invalidate availability |
| 36 | Cache | `app/routers/rooms.py` | Room create doesn't invalidate report |
| 37 | Cache | `app/routers/admin.py` | Inverted date range returns 200 |
| 38 | Robustness | `app/auth.py` | Revoked token set grows unbounded |
| 39 | Input | `app/routers/bookings.py` | Malformed datetime → 500 |
| 40 | Input | `app/services/export.py` | CSV uses \r\n line endings |
| 41 | Cache | `app/cache.py` | Stale-cache TOCTOU race condition |
| 42 | Input | `app/routers/bookings.py` | Broken notifications import on router |
| 43 | Booking | `app/routers/bookings.py` | Admin list_bookings query filters by user_id |
